...
(begin-encourage-inline
  ...
  (define/contract
    ...
    (parametric->/c (A) (-> (vectorof A) (vectorof A)))
  ...
  (define;/contract
    (array-shape-size ds)
    ;(parametric->/c (A) (-> (vectorof A) natural-number/c))
  ...
  (define;/contract
    ...
    ;(parametric->/c (A) (-> symbol? (vectorof A) natural-number/c))
  ...
  (define;/contract
    ...
    ;(parametric->/c (A) (-> (vectorof A) (-> none/c) (vectorof A)))
    (define dims (vector-length ds))
  ...
  (define/contract
    ...
    (parametric->/c (A) (-> (vectorof A) (vectorof A) natural-number/c))
    (define dims (vector-length ds))
    (let loop ([i 0] [j 0])
      (cond [(i . < . dims)
             (define di (unsafe-vector-ref ds i))
...
(define/contract
  ...
  (parametric->/c (A) (-> symbol? (vectorof A) any/c none/c))
...
(define/contract
  ...
  (parametric->/c (A) (-> (vectorof A) natural-number/c (vectorof A)))
...
(define/contract
  ...
  (parametric->/c (A) (-> (vectorof A) natural-number/c (vectorof A) void?))
  (let loop ([k  dims])
    (unless (zero? k)
      (let ([k  (- k 1)])
