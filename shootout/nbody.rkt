...
;; -------------------------------
(define/contract (offset-momentum)
  (-> void?)
  (define/contract (loop-i i px py pz)
    (-> (listof body?) number? number? number? void?)
    (if (null? i)
      (begin
  ...
  (loop-i *system* 0.0 0.0 0.0)
                )

;; -------------------------------
(define/contract (energy)
  (-> number?)
  (define/contract (loop-o o e)
    (-> (listof body?) number? number?)
    (if (null? o)
      e
      (let* ([o1 (car o)]
             [e (+ e (fl* 0.5 
        ...
        (loop-i (cdr o) e o o1))))
  (define/contract (loop-i i e o o1)
    (-> (listof body?) number? (listof body?) body? number?)
          (if (null? i)
            (loop-o (cdr o) e)
            (let* ([i1   (car i)]
  ...
  (define/contract (loop-o o)
    ...
    (when (pair? o)
      (let* ([o1  (car o)]
             ...
             [o1z (body-z o1)]
             [om  (body-mass o1)])
        (loop-i (cdr o) (body-vx o1) (body-vy o1) (body-vz o1) o1 o1x o1y o1z om)))
      (loop-o (cdr o)))
   (define/contract (loop-i i vx vy vz o1 o1x o1y o1z om)
     (-> (listof body?) number? number? number? body? number? number? number? number? void?)
          (if (pair? i)
            (let* ([i1    (car i)]
                   [dx    (fl- o1x (body-x i1))]
                   [dy    (fl- o1y (body-y i1))]
              ...
              (loop-i (cdr i)
                      (fl- vx (fl* dxmag im))
                      (fl- vy (fl* dymag im))
                      (fl- vz (fl* dzmag im)) o1 o1x o1y o1z om))
...
(time (begin (main 30) (void)))
; (time (begin (main 3000000) (void)))
